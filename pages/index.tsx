...
const FAQS: { q: string; a: string }[] = [
  {
    ...
    a: `이해하기 쉽게 예시를 들어보겠습니다.

헤모글로빈이 7g/dL 이하 일 때 욕창 발생 확률이 40%, 체온이 38도 이상일 때 욕창 발생 확률이 50%라고 해봅시다. 헤모글로빈 7g/dL 이면서 체온이 38도 이상인 환자의 욕창 발생 확률은 40% * 50% = 20%가 됩니다.
(전체적인 느낌을 설명하기 위한 예시이므로 참고만 해주세요)

추후 업데이트에 따라 사용하는 머신러닝 알고리즘이 달라질 수 있고 각 알고리즘마다 확률을 도출하는 방식이 상이하여 일괄적으로 설명드리기 어려운 점 양해부탁드립니다.`,
  },
  {
    q: 'Q2. 각 변수의 기여도는 어떻게 계산되는 건가요?',
    a: `머신러닝 모델이 어떻게 결론에 도달했는지 알려주는 변수별 기여도 분석을 사용했습니다. 예측 결과에 영향을 준 요소들을 점수로 매긴 뒤, 위험을 끌어올린 변수들 사이의 상대 비율(%)로 표시합니다. 따라서 표시된 % 의 합은 항상 100% 가 됩니다.`,
  },
  {
    q: 'Q3. 이 프로그램은 얼마나 정확한가요?',
    a: `이 모델은 서울아산병원 중환자실에 입실한 22,000여 명의 환자 데이터를 학습하였으며 정확도는 테스트 데이터 기준으로 87.7% (AUC 0.887) 로 측정되었습니다.

정확도를 이해하기 위해 임계점을 알아야 합니다. 임계점이란 쉽게 말해 머신러닝 모델이 해당 환자를 욕창군으로 판단하는 최소 위험도값입니다.
...
이 도구에서는 Decision Curve Analysis 결과에 따라 50점 이상을 욕창 위험군으로 권고합니다.
...
그러나 내부 논의를 거쳐 간호사가 직접 중재를 제공하며 욕창을 예방하는 활동에 참여할 수 있는 변수만 다시 선정하여 모델을 구축하였습니다. 최종 선정한 변수는 의식수준, 하지근력, 체온, 실금이며 모델의 정확도는 이 변수만으로 욕창을 예측했을 때 계산하였습니다.`,
  },
  {
    ...
    a: `아니요. 딥러닝은 머신러닝의 하위분야로 머신러닝보다 더 복잡한 현상을 예측하는데 최적화되어있습니다.

구체적으로 설명드리면 알파고에서 사용한 모델은 '인공신경망', 이 프로젝트에서 사용한 모델은 'Gaussian Naïve Bayes' 입니다. 참고로 이 모델은 PyCaret 을 사용하여 수백개의 머신러닝 모델 중 가장 성능이 뛰어난 모델을 선정하였으며 추후 모델링에 사용하는 데이터가 변경됨에 따라 더 성능이 좋은 알고리즘으로 교체될 수 있습니다.`,
  },
  {
      ...
      <div className="container mt-5">
        ...
        <div className="usage-guide mt-4 p-4 bg-white shadow rounded">
          ...
          <ol className="pl-3" style={{ fontSize: 20 }}>
            ...
            <li>
              홈페이지를 처음 켤 때나 한동안 사용하지 않은 후 첫 요청에는 모델 로딩으로 약 5~10초 정도 시간이 소요될 수 있습니다. 이후에는 즉시 결과가 나옵니다.
            </li>
            <li>
